# Contributing to the YON TextMate grammar

Thank you for your interest in contributing to the YON TextMate grammar, a
YounndAI™ product by MARLINK TRADING SRL.

This repository contains the TextMate grammar and Shiki integration for YON
syntax highlighting — editor support for the
[YON specification](https://github.com/YounndAI/yon-spec). It is published to
npm as `@younndai/yon-textmate`.
...
npm only — never pnpm/yarn/bun; Node >= 20.9.

```bash
npm install   # install dependencies
npm run build # compile the Shiki integration (tsc + license banner)
npm test      # run the grammar tests (vscode-tmgrammar-test)
```

Grammar changes live in `syntaxes/yon.tmLanguage.json`; every change should be
covered by a test case under `tests/`.
